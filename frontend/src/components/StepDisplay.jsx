...
const StepDisplay = ({ stepId, onAnswer }) => {
  ...
  const stepDataFromState = stepsFetched[stepId];
  const {
  ...
  const { steps } = useSelector((state) => state.quiz?.currentQuiz);
  ...
  const handleNext = () => {
    if (currentStep !== totalSteps) {
      ...
      const nextStepId = steps[currentStep]; // Get the previous step ID
  ...
  const handleSent = () => {
    if (!!stepsFetched[lastStep]?.answer) {
  ...
  const handleBack = () => {
    ...
    if (currentStep > 1) {
      const previousStepId = steps[currentStep - 2]; // Get the previous step ID
  ...
  const isAnswered = !!stepsFetched[lastStep]?.answer;
  // Render input based on question type.
  const renderInput = () => {
    const givenAnswer = stepsFetched[lastStep]?.answer; // Get the answer from Redux state
